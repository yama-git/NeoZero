...
const ChangePass = () => {
  ...
  const handleok = async () => { // 「OK」ボタン押下
    ...
    try {
      const response = await fetch('https://neozero.metifie.com/userinfo/pass/change', {
        method: 'PUT',
        headers: {
  ...
  const handlead1 = () => {
    //外部サイトへ飛ぶ(新しいタブで)
    window.open('https://www.info.kochi-tech.ac.jp/faculty_members/profile_mendori.shtml', '_blank', 'noopener noreferrer')
  };
